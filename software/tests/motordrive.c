...
int main(int argc, char *argv[])
{
  ...
  for(int i = 0; i < WAVEFORM_POINTS; i++)
  {
    ...
    if(sine_amplitude >= 0)
    {
      ...
      gpio = 1 << (AZ_CW_P) | 1 << (EL_CW_P);
    }
    else
    {
      gpio = 1 << (AZ_CW_N) | 1 << (EL_CW_N);
    }
  ...
  gpioPulse_t *dir_b_wf = (gpioPulse_t *)malloc(WAVEFORM_POINTS * sizeof(gpioPulse_t));
  if(dir_b_wf == NULL)
  {
    fprintf(stderr, "Error allocating local memory for waveform dir_b_wf\n");
    return 1;
  }
  ...
  for(int i = 0; i < WAVEFORM_POINTS; i++)
  {
    ...
    {
      /* Positive half of wave */
      gpio = 1 << (AZ_CCW_P) | 1 << (EL_CCW_P);
    }
    else
    {
      gpio = 1 << (AZ_CCW_N) | 1 << (EL_CCW_N);
    }
    ...
    if((i & 0x01) == 0x00)
    {
      dir_b_wf[i].gpioOn = gpio;
      dir_b_wf[i].gpioOff = 0;
      dir_b_wf[i].usDelay = (int)(round(duty_cycle * WAVEFORM_PERIOD_US));
      printf("%06d (%f dc)\n", dir_b_wf[i].usDelay, duty_cycle);
    }
    else
    {
      dir_b_wf[i].gpioOn = 0;
      dir_b_wf[i].gpioOff = gpio;
      dir_b_wf[i].usDelay = WAVEFORM_PERIOD_US - (int)(round(duty_cycle * WAVEFORM_PERIOD_US));
    }
  }

  int dir_b_points = gpioWaveAddGeneric(WAVEFORM_POINTS, dir_b_wf);
  printf("Dir B Pulses: %d\n", dir_b_points);

  int dir_b_wave __attribute__((unused)) = gpioWaveCreate();
