...
#define EL_HYSTERESIS   (1.0)

#define AZ_RELAY_PAUSE_MS   (200)
#define EL_RELAY_PAUSE_MS   (200)
...
void *loop_motordrive(void *arg)
{
  ...
  while(!(app_state->app_exit))
  {
    if(app_state->demand_valid
      && app_state->encoder_valid
    )
    {
      app_state->error_az_deg = app_state->encoder_az_deg - app_state->demand_az_deg;
      app_state->error_el_deg = app_state->encoder_el_deg - app_state->demand_el_deg;
      app_state->error_lastupdated_ms = timestamp_ms();
      app_state->error_valid = true;

      // Check Azimuth
      if(app_state->error_az_deg > AZ_HYSTERESIS
      ...
      else if(app_state->error_az_deg < (-1 * AZ_HYSTERESIS)
        && (az_relay_lastswitch + AZ_RELAY_PAUSE_MS) < timestamp_ms())
      {
        if(az_relay_state == 1)
        {
      ...
      else
      {
        ...
        if(pwm_state == WAVE_AZ || pwm_state == WAVE_AZEL)
        {
          ...
          {
            pwm_state = WAVE_OFF;
            app_state->demand_flag = false;
          }
        }
      }

      // Check Elevation
      if(app_state->error_el_deg > EL_HYSTERESIS
      ...
      else if(app_state->error_el_deg < (-1 * EL_HYSTERESIS)
        && (el_relay_lastswitch + EL_RELAY_PAUSE_MS) < timestamp_ms())
      {
        if(el_relay_state == 0)
        {
